...
var App = function() {
		...
		jQuery('.page-content')
				.on(
						...
						function(e) {
							...
							var pageContent = $('.page-content');
							var pageContentBody = $(pageContent).find(
									'.page-content-body');
							...
							var multipart = ($(this).attr('enctype') == mp || $(
									this).attr('encoding') == mp);
							if (multipart) {
							} else {
								App.blockUI(pageContent, false);
								$.ajax({
	...
	var handleSelectAll = function() {
		...
		jQuery('body').on(
				'change',
				'.table-checkable thead th.checkbox-column :checkbox',
				function() {
					var checked = $(this).prop('checked');
					$(this).parents('table').children('tbody').each(
							function(i, tbody) {
								$(tbody).find('.checkbox-column').each(
										function(j, cb) {
											$(cb).find(':checkbox').prop(
													"checked", checked)
													.trigger('change');
										});
							});
		...
		jQuery('body').on('change',
				'.table-checkable tbody tr td.checkbox-column :checkbox',
				function() {
					var checked = $(this).prop('checked');
	...
	var handleTheme = function() {
		...
		var setColor = function(color) {
			$('#style_color').attr("href",
					themeRoot + "/assets/css/themes/" + color + ".css");
